...
import { ChatScrollbarGeometry, ChatScrollbarPosition } from './scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
  selector: '[chatScrollbar]',
  exportAs: 'chatScrollbar',
})
export class ChatScrollbarDirective implements OnChanges, OnInit, OnDestroy {
  /* eslint-disable @typescript-eslint/naming-convention */
  static ngAcceptInputType_chatScrollbar: BooleanInput;
  /* eslint-enable @typescript-eslint/naming-convention */

  @Input() chatScrollbar = true;
  @Input() chatScrollbarOptions: PerfectScrollbar.Options | undefined;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Enabled
    if ('chatScrollbar' in changes) {
      // Interpret empty string as 'true'
      this.chatScrollbar = coerceBooleanProperty(changes['chatScrollbar'].currentValue);
    ...
    // Scrollbar options
    if ('chatScrollbarOptions' in changes) {
      // Merge the options
      this._options = merge({}, this._options, changes['chatScrollbarOptions'].currentValue);
   ...
   * @param prefix
   */
  geometry(prefix: string = 'scroll'): ChatScrollbarGeometry {
    return new ChatScrollbarGeometry(
   ...
   * @param absolute
   */
  position(absolute: boolean = false): ChatScrollbarPosition {
    let scrollbarPosition;

    if (!absolute && this._ps) {
      scrollbarPosition = new ChatScrollbarPosition(this._ps.reach.x || 0, this._ps.reach.y || 0);
    } else {
      scrollbarPosition = new ChatScrollbarPosition(
