...
export class ChatNavigationService {
  ...
  private _navigationStore: Map<string, ChatNavigationItem[]> = new Map<string, any>();
   ...
   * @param navigation
   */
  storeNavigation(key: string, navigation: ChatNavigationItem[]): void {
    // Add to the store
   ...
   * @param key
   */
  getNavigation(key: string): ChatNavigationItem[] {
   ...
   */
  getFlatNavigation(navigation: ChatNavigationItem[], flatNavigation: ChatNavigationItem[] = []): ChatNavigationItem[] {
   ...
   * @param navigation
   */
  getItem(id: string, navigation: ChatNavigationItem[]): ChatNavigationItem | null {
    for (const item of navigation) {
      if (item.id === id) {
        return item;
      }
  ...
  getItemParent(
    ...
    parent: ChatNavigationItem[] | ChatNavigationItem
  ): ChatNavigationItem[] | ChatNavigationItem | null {
    for (const item of navigation) {
      if (item.id === id) {
        return parent;
      }
